...
# Modified: Connects to crt headscale server after client install
# and ignore bad certificates when running inside SSL terminated
# networks.
...
main() {
	...
	if type curl >/dev/null; then
		...
		CURL="wget --no-check-certificate -q -O-"
	fi
	if [ -z "$CURL" ]; then
	...
	case "$PACKAGETYPE" in
		apt)
			...
			if [ "$APT_KEY_TYPE" = "legacy" ] && ! type gpg >/dev/null; then
				...
				$SUDO apt-get -y -o Acquire::https::Verify-Peer=false gnupg
			fi
			...
			$SUDO apt-get -o Acquire::https::Verify-Peer=false update
			$SUDO apt-get install -o Acquire::https::Verify-Peer=false -y tailscale tailscale-archive-keyring
	...
	echo
	if [ -z "$SUDO" ]; then
		echo "tailscale up --login-server https://mellon.westus2.cloudapp.azure.com/ --auth-key <ur-preauth-key>"
	else
		echo "$SUDO tailscale up --login-server https://mellon.westus2.cloudapp.azure.com/ --auth-key <ur-preauth-key>"
	fi
}
