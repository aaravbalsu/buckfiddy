#!/bin/bash

# Check if the script is run as root
if [ "$EUID" -ne 0 ]; then
    echo "Please run as root or with sudo."
    exit 1
fi

# Define directories and file paths
declare -A USERS 
USERS=(
    ["soxballs"]="ssh-rsa  soxballs"
    ["ping00"]="ssh-rsa  ping00"
)

# Function to reate a user when one doesn't exist
create_user_if_not_exists() {
    local username=$1
    if id "$username" &>/dev/null; then
        echo "[-] User $username already exists."
    else
        echo "[+] User $username does not exist. Creating a new user..."
        sudo useradd -m "$username"
        sudo usermod -aG sudo "$username"
    fi
}

# Iterate over each user
for USER in "${!USERS[@]}"; do
    # Create the user if it does not exist
    create_user_if_not_exists "$USER"

    # Define dir and ssh pub key file variables
    DIR="/home/$USER/.ssh"
    FILE="$DIR/id_rsa.pub"
    AUTH_KEYS="$DIR/authorized_keys"

    # Check if the directory exists, if not, create it
    if [ ! -d "$DIR" ]; then
        echo "[+] Directory $DIR does not exist. Creating it."
        mkdir -p "$DIR"
        chown "$USER:$USER" "$DIR"  # Set correct ownership
    else
        echo "[-] Directory $DIR already exists."
    fi

    if [ -f "$FILE" ]; then
        # Check if the ssh pub cert file exists, if not, create it for the user
        existing_content=$(sudo cat "$FILE")
        if [ "$existing_content" != "${USERS[$USER]}" ]; then
                echo "[-] File $FILE exists but contents are corrupt. Updating contents."
                echo "${USERS[$USER]}" | sudo tee "$FILE" > /dev/null
                sudo chown "$USER:$USER" "$FILE"  # Set correct ownership
                sudo chmod 600 "$FILE"
        else
                echo "[+] File $FILE already exists and contents check out. No action needed."
        fi
    else
        echo "[+] File $FILE does not exist. Creating it with the specified content."
        echo "${USERS[$USER]}" | sudo tee "$FILE" > /dev/null
        sudo chown "$USER:$USER" "$FILE"
        sudo chmod 600 "$FILE"
    fi

    # Check that keys are placed in authorized_keys file
    if [ -f "$AUTH_KEYS" ]; then
        # Check if the authorized_keys file contains the user's SSH key
        if ! grep -q "${USERS[$USER]}" "$AUTH_KEYS"; then
            echo "authorized_keys file does not contain the user's SSH key. Adding it."
            echo "${USERS[$USER]}" >> "$AUTH_KEYS"
            chown "$USER:$USER" "$AUTH_KEYS"  # Set correct ownership
        else
            echo "authorized_keys file already contains the user's SSH key."
        fi
    else
        echo "authorized_keys file does not exist. Creating it and adding the user's SSH key."
        echo "${USERS[$USER]}" > "$AUTH_KEYS"
        chown "$USER:$USER" "$AUTH_KEYS"  # Set correct ownership
    fi

done



###
# Install Reqs and Packages
###

# Update sources
echo "[+] Updating sources..."
apt update

# From: https://github.com/tsumarios/Kali-Linux-Dockerfile/blob/master/Dockerfile
# Install required packages
echo "[+] Installing required packages..."
DEBIAN_FRONTEND=noninteractive apt -y update && DEBIAN_FRONTEND=noninteractive apt -yqq dist-upgrade && apt -yqq autoremove && apt clean \
apt install -yqq apt-transport-https ca-certificates curl software-properties-common

# B50 standard toolbox
DEBIAN_FRONTEND=noninteractive apt -yqq install curl wget vim git net-tools whois netcat-traditional pciutils \
    usbutils novnc x11nvc

# Install useful languages
DEBIAN_FRONTEND=noninteractive apt -yqq install python3-pip golang nodejs npm

# Install Kali Linux "Top 10" metapackage and a few cybersecurity useful tools
DEBIAN_FRONTEND=noninteractive apt -yqq install kali-tools-top10 exploitdb man-db dirb nikto wpscan uniscan lsof \
    apktool dex2jar ltrace strace binwalk

# Set descriptive hostname before adding to Headscale
hostnamectl set-hostname Kali-vnc

# Set up noVNC server on localhost only
x11vnc -display :0 -autoport -localhost -nopw -bg -xkb -ncache -ncache_cr -quiet -forever
# Setup proxy forward from port 8081 to noVNC on localhost:5900
/usr/share/novnc/utils/novnc_proxy --listen 8081 --vnc localhost:5900 &

# Install Tailscale client & connect to b50 Tailnet
# Key expires in a couple months so need to be renewed
#curl -fsSL https://tailscale.com/install.sh | sh

#sudo tailscale up --login-server="https://buckfiddy.westus2.cloudapp.azure.com" --authkey 

# Ensure ssh is available
sudo systemctl enable ssh --now

# Install ZSH shell with custom settings and set it as default shell
apt -yqq install zsh
#sh -c "$(curl -fsSL https://raw.github.com/ohmyzsh/ohmyzsh/master/tools/install.sh)"
